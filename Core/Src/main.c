...
int main(void)
{
  ...
  static uint32_t lid_timer_tick = 0;
  /* USER CODE END 1 */
  ...
  while (1)
  {
      ...
      switch (lid_state)
      {
        case LID_CLOSED:
          if (dist < 100 && dist > 20)
          {
            ...
            lid_timer_tick = HAL_GetTick();      // record tick of opening lib
        ...
        case LID_OPENING:
          ...
          {
            _detection_count++;
            if (_detection_count >= 5)
            {
              lid_timer_tick = HAL_GetTick();  // reset counter
          ...
          if ((HAL_GetTick() - lid_timer_tick >= LID_HOLD_TIME) && (_detection_count == 0))
          {
            sg90_set_angle(&sg90, (uint16_t)LID_CLOSE_ANGLE);
            lid_timer_tick = HAL_GetTick();
    ...
    {
      if (HAL_GetTick() - lid_timer_tick >= LID_CLOSING_TIME)
      {
